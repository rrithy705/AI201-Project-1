...
class _OnnxEmbedder:
    ...
    def __init__(self):
        ...
        # Pinned to CPU on purpose. Left to itself, onnxruntime 1.23 picks
        # CoreMLExecutionProvider first on this Mac and then dies partway
        # through a batch with "Unable to compute the prediction using a
        # neural network model", which reads like a broken model and isn't —
        # test.py passes because it only embeds a couple of strings, and
        # indexing 94 chunks is what trips it. CPU is the provider that
        # actually works here, and at this corpus size the speed difference
        # is not measurable.
