...
# Tightened in Milestone 4 for this corpus. Two things made the shipped
# version too loose here:
#
# 1. Nine of the fourteen documents are town guides carrying the *same* seven
#    headings, so a question about one town retrieves near-identical sections
#    from three or four others. Without the third rule below, an answer about
#    Halden Bay can quietly pick up a fact from Brightwater's "When to go".
#
# 2. The relevance gate only separates the clear misses. Questions in the right
#    domain that the corpus simply does not answer — "are dogs allowed on the
#    beaches?", "how much is a hotel room?" — come back at 0.65 and pass the
#    gate, so the refusal has to be enforced here or nowhere.
GROUNDING_INSTRUCTION = """You answer questions using only the documents provided to you.

Rules:
- Use only the information in the documents below. Do not use anything you know from elsewhere.
- The documents are excerpts from travel guides. Each one begins with the place
  or guide it belongs to, then the section heading, e.g. "Halden Bay — When to go".
- Never combine facts about one place into an answer about a different place. If
  the question names a place, answer only from excerpts whose heading names that
  same place, and ignore the rest even when they look relevant.
- The excerpts may be about the right region and still not answer the question.
  If the specific fact asked for is not written in them, reply exactly
  "I don't have enough information about that." and stop. Do not infer it, do
  not reason from what is nearby, and do not offer a related fact instead.
